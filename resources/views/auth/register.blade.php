...
.step-circle {
  ...
  transition: all .3s cubic-bezier(.34,1.2,.64,1); position: relative; z-index: 1;
}
...
.peran-card {
  border: 1.5px solid #e8eaf0; border-radius: 12px; padding: 12px;
  cursor: pointer; transition: all .18s; background: #f8fafc; text-align: left;
}
...
.kantor-card {
  border: 1.5px solid #e8eaf0; border-radius: 11px; padding: 10px 13px;
...
.kantor-radio { width: 18px; height: 18px; border-radius: 50%; border: 2px solid #cbd5e1; display: flex; align-items: center; justify-content: center; transition: all .18s; flex-shrink: 0; }
.kantor-card.selected .kantor-radio { border-color: #f97316; background: #f97316; }
.kantor-radio-dot { width: 7px; height: 7px; border-radius: 50%; background: #fff; opacity: 0; transition: opacity .15s; }
...
.alert-error {
  ...
  padding: 11px 14px; margin-bottom: 14px; font-size: 12.5px; color: #991b1b;
...
.info-badge {
  ...
  border: 1px solid #bfdbfe; border-radius: 10px; padding: 10px 13px;
  font-size: 11.5px; color: #1e40af; display: flex; align-items: center; gap: 7px;
}
  ...
  <section class="panel-left">
    ...
    <div style="position:relative;z-index:1;">
      ...
      <h1 style="font-weight:800;font-size:40px;color:#fff;line-height:1.1;margin-bottom:18px;letter-spacing:-.02em;">
        Bergabung.<br/>
        <span style="color:#f97316;">Daftarkan</span><br/>
        Akunmu.
      </h1>
      <p style="color:#94a3b8;font-size:14px;line-height:1.75;max-width:320px;">
        Ajukan akses ke sistem inventaris PT. Dian Pilar Utama. Pengajuan akan ditinjau oleh Admin sebelum akun diaktifkan.
      </p>
      ...
      <div style="margin-top:32px;display:flex;flex-direction:column;gap:10px;">
        <div style="display:flex;align-items:center;gap:10px;">
          <div style="width:30px;height:30px;border-radius:8px;background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <span class="material-symbols-outlined fill-icon" style="color:#f97316;font-size:15px;">verified_user</span>
          </div>
          <span style="font-size:13px;color:#94a3b8;font-weight:500;">Pengajuan diverifikasi Admin</span>
        </div>
        <div style="display:flex;align-items:center;gap:10px;">
          <div style="width:30px;height:30px;border-radius:8px;background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <span class="material-symbols-outlined fill-icon" style="color:#f97316;font-size:15px;">lock</span>
          </div>
          <span style="font-size:13px;color:#94a3b8;font-weight:500;">Data aman & terenkripsi</span>
        </div>
        <div style="display:flex;align-items:center;gap:10px;">
          <div style="width:30px;height:30px;border-radius:8px;background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <span class="material-symbols-outlined fill-icon" style="color:#f97316;font-size:15px;">apartment</span>
          </div>
          <span style="font-size:13px;color:#94a3b8;font-weight:500;">Akses sesuai kantor yang dipilih</span>
        </div>
      </div>
    </div>
    ...
    <div class="form-wrap fade-up">
      ...
      <div class="reg-card">
        ...
        <form method="POST" action="{{ route('register.store') }}" id="reg-form">
          ...
          <input type="hidden" name="kantor_id" id="f-kantor-id"/>

          <!-- ══ STEP 1: Nama + Email + Password ══ -->
          <div id="step1" class="panel-anim in" style="display:flex;flex-direction:column;gap:14px;">
            ...
            <div>
              ...
              <div style="position:relative;">
                ...
                <input id="inp-nama" name="nama" type="text" placeholder="Nama lengkap Anda"
                  class="field {{ $errors->has('nama') ? 'error' : '' }}"
                  style="padding-left:40px;" value="{{ old('nama') }}"/>
              </div>
            ...
            <div>
              ...
              <div style="position:relative;">
                ...
                <input id="inp-email" name="email" type="email" placeholder="email@dianpilar.co.id"
            ...
            <div>
              ...
              <div style="position:relative;">
                ...
                <input id="inp-pass" type="password" name="password" placeholder="Min. 8 karakter"
                  class="field {{ $errors->has('password') ? 'error' : '' }}"
                  style="padding-left:40px;padding-right:42px;" oninput="checkStrength(this.value)"/>
            ...
            <div>
              ...
              <div style="position:relative;">
                ...
                <input id="inp-pass2" type="password" name="password_confirmation" placeholder="Ulangi kata sandi"
                  class="field" style="padding-left:40px;padding-right:42px;" oninput="checkMatch()"/>
              ...
              <div id="match-label" class="field-error" style="display:none;">
                <span class="material-symbols-outlined fill-icon" style="font-size:12px;">error</span>
          ...
          <div id="step2" class="panel-anim" style="display:none;flex-direction:column;gap:14px;">
              ...
              <div class="peran-grid">
                <div class="peran-card" id="card-operator" onclick="selectPeran('operator')">
                  <div class="peran-icon">
                    <span class="material-symbols-outlined fill-icon" style="font-size:16px;color:#64748b;">badge</span>
                  </div>
                  <div class="peran-title">Operator</div>
                  <div class="peran-desc">Akses terbatas satu kantor. Kelola inventaris harian.</div>
                </div>
                <div class="peran-card" id="card-admin" onclick="selectPeran('admin')">
                  <div class="peran-icon">
                    <span class="material-symbols-outlined fill-icon" style="font-size:16px;color:#64748b;">admin_panel_settings</span>
                  </div>
            ...
            <div style="display:flex;gap:8px;">
              <button type="button" onclick="goStep1()" class="btn-ghost">
          ...
          <div id="step3" class="panel-anim" style="display:none;flex-direction:column;gap:14px;">
            ...
            <!-- Kantor (tampil kalau operator) -->
            <div id="kantor-section" style="display:none;">
              <label class="field-label" style="margin-bottom:8px;">Pilih Kantor</label>
              <div class="kantor-list" id="kantor-list">
                @foreach($kantorList as $k)
                <div class="kantor-card" data-id="{{ $k->id }}" data-nama="{{ $k->nama }}" onclick="selectKantor(this)">
                  <div>
              ...
              <div id="kantor-error" class="field-error" style="display:none;margin-top:6px;">
                <span class="material-symbols-outlined fill-icon" style="font-size:12px;">error</span>Pilih kantor terlebih dahulu.
              </div>
            </div>
            ...
            <div>
              ...
              <textarea id="inp-alasan" name="alasan" placeholder="Jelaskan alasan Anda membutuhkan akses sistem inventaris ini..." class="field {{ $errors->has('alasan') ? 'error' : '' }}">{{ old('alasan') }}</textarea>
            ...
            <div style="display:flex;gap:8px;">
              <button type="button" onclick="goStep2b()" class="btn-ghost">
                <span class="material-symbols-outlined" style="font-size:18px;">arrow_back</span>
              </button>
              <button type="submit" class="btn-or" style="flex:1;" id="submit-btn" onclick="return validateStep3()">
                <span class="material-symbols-outlined fill-icon" style="font-size:18px;">how_to_reg</span>
                Kirim Pengajuan
              </button>
            </div>
          </div>
      ...
      <p style="text-align:center;font-size:11.5px;color:#94a3b8;margin-top:16px;">
        Sudah punya akun?
        <a href="{{ route('login') }}" style="color:#f97316;font-weight:700;text-decoration:none;">Masuk di sini</a>
      </p>
...
function goStep3() {
  if (!selectedPeran) {
    document.getElementById('peran-error').style.display = 'flex';
    return;
  }
  ...
  // Tampil/sembunyikan kantor section
  document.getElementById('kantor-section').style.display =
    selectedPeran === 'operator' ? 'block' : 'none';
...
// ── Pilih Kantor ───────────────────────────────────────────────
function selectKantor(el) {
  document.querySelectorAll('.kantor-card').forEach(function(c) { c.classList.remove('selected'); });
  el.classList.add('selected');
  selectedKantor = el.dataset.id;
  document.getElementById('f-kantor-id').value = selectedKantor;
  document.getElementById('kantor-error').style.display = 'none';
}

// ── Validate Step 3 sebelum submit ────────────────────────────
function validateStep3() {
  if (selectedPeran === 'operator' && !selectedKantor) {
...
// ── Restore state dari old() kalau server error ────────────────
@if(old('peran'))
  ...
  @if(old('kantor_id'))
    var oldKantor = document.querySelector('.kantor-card[data-id="{{ old('kantor_id') }}"]');
    if (oldKantor) selectKantor(oldKantor);
  @endif
@endif
</script>
</body>
